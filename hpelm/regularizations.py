...
def semi_Tikhonov(X, T):
    # add semi-Tikhonov regularization: small random noise projected to "zero"
    # "zero" = zero + E[T], otherwise we introduce a bias
    nT = X.shape[0]/10 + 1
    tkX = np.random.rand(nT, X.shape[1]-1) * 10E-6
    tkX = np.hstack((tkX, np.ones((nT,1))))
    tkT = np.tile(T.mean(0), (nT,1))
    X = np.vstack((X, tkX))
    T = np.vstack((T, tkT))
    return X, T
...
def mrsr(X, T, kmax):
    ...
    | **W**    is an (m x p*kmax) sparse matrix of regression
          ...
          full(W). Regression coefficients of the k:th step ar e given
          by W(:,(k-1)*p+1:k*p).
