...
cdef extern from "mic_code.h":
    cdef void solve_corr (int n, int nrhs, double* A, double* B, double* X)
    cdef cppclass MicSolver:
        MicSolver( int, int, double* A, double* B) except +
...
cdef class MicSolver:

    cdef MicSolver *solverptr
    cdef int n, nrhs
    ...
    def __init__(self, nn, outs, norm=1E-9):
        ...
        self.solverptr = new MicSolver( self.n, self.nrhs, &A[0,0], &B[0,0] )
...
def mic_solve(np.ndarray[double, ndim=2, mode="c"] XX not None,
