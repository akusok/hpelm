...
class Solver(object):
    ...
    def solve(self):
        ...
        """                
#        HH = np.asfortranarray(np.triu(self.HH))
#        HT = np.asfortranarray(self.HT)

        _, B, info = lapack.dposv(self.HH, self.HT)  # only well cond, super fast
        if info != 0:
            HH = self.HH + np.triu(self.HH, k=1).T        
            try:
                B = np.linalg.solve(HH, self.HT)  # ill cond but not singular, fast
                print "ill conditioned matrix - adjust input data and random weights range"
                print "or maybe you have more neurons than unique training samples"
            except np.linalg.linalg.LinAlgError:
                print "singular matrix - check for duplicate neurons"
                print "solution is possible, but slow for large number of neurons"
                B = np.linalg.lstsq(HH, self.HT)[0]  # works for singular, very slow 

        self.B = B

#        B.append(scipy.linalg.solve(HHf, HTf))               # ill cond, fast
#        wrk, iwrk, info = lapack.dgelsd_lwork(self.L, self.L, self.c)
#        B.append(lapack.dgelsd(HHf, HTf, wrk, int(iwrk))[0]) # singular and ill cond, very slow 
#        B.append(np.dot(np.linalg.pinv(HHf), HTf))           # singular and ill cond, very slow 
...
if __name__ == "__main__":
    ...
    W = np.random.randn(d, L)*3/d**0.5
    B = np.random.rand(L)
    W1 = np.random.randn(d, L)*3/d**0.5
    B1 = np.random.rand(L)

    neurons = [('tanh', L, W, B),
               ('sigm', L, np.random.randn(d, L)*3/d**0.5, np.random.rand(L)),
               ('tanh', L, W, B)]
    ...
    sl.HH = np.asfortranarray(np.triu(sl.HH))
    sl.HT = np.asfortranarray(sl.HT)
    sl.solve()
    Y = sl.predict(X)
    print np.mean((T - Y)**2)
    print "Done!"
