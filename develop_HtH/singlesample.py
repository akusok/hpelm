...
class elm(object):
    ...
    def train(self, X, T, nn, idx):
        ...
        Hneg = H[T[:,3]==1]
        Tneg = T[T[:,3]==1]
        HHneg = np.dot(Hneg.T, Hneg) + 1E-6*np.eye(Hneg.shape[1])
        HTneg = np.dot(Hneg.T, Tneg)    
        ...
        k = k / np.sum(k)
        HH = k[0]*HH1 + k[1]*HH2 + k[2]*HH3 + 0.01*HHneg
        HT = k[0]*HT1 + k[1]*HT2 + k[2]*HT3 + 0.01*HTneg
...
def code_show(nn, i):
    ...
    n,d = Xtr.shape
    Nneg = n*10
    Yneg = np.tile(np.array([[0,0,0,1]]), (Nneg,1))
    Xneg = np.zeros((Nneg, d))
    s = (np.std(Xtr[Ytr[:,0]==1]) + np.std(Xtr[Ytr[:,1]==1]) + np.std(Xtr[Ytr[:,2]==1])) / 3
    for i in xrange(Nneg):
        sample = Xtr[np.random.randint(n)] + np.random.randn(1,2)*0.1
        Xneg[i] = sample
        #while True:
            #dist = cdist(sample, Xtr, "euclidean")
            #if np.min(dist) > s:
            #if np.sum(sample**2)**0.5 > 3:
            #    Xneg[i] = sample
            #    break

    Ytr = np.hstack((Ytr, np.zeros((n,1))))
    Ytr = np.vstack((Ytr, Yneg))
    Xtr = np.vstack((Xtr, Xneg))
    m2 = Xtr.mean(0)
    s2 = Xtr.std(0)
    #Xtr = (Xtr - m2) / s2
    ...
    # prepare mesh grid
    sz = 8
    x = np.arange(-sz, sz, 0.1)
    y = np.arange(-sz, sz, 0.1)
    n = x.shape[0]
    ...
    plt.imshow(I, extent=[-sz, sz, -sz, sz])
    plt.xlim([-sz, sz])
    plt.ylim([-sz, sz])
    #cl = 1
    ...
    plt.scatter(Xtr[Ytr[:,1]==1,0], Xtr[Ytr[:,1]==1,1], c=Ytr[Ytr[:,1]==1])
    #plt.scatter(Xtr[idx,0], Xtr[idx,1], c=Ytr[idx])
    plt.scatter(Xtr[Ytr[:,2]==1,0], Xtr[Ytr[:,2]==1,1], c=Ytr[Ytr[:,2]==1])
    plt.scatter(Xtr[Ytr[:,3]==1,0], Xtr[Ytr[:,3]==1,1], marker='+', c=[0,0,0])
    plt.show()
